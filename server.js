...
db.serialize(() => {
  ...
  db.get('SELECT COUNT(*) as count FROM classes', (err, row) => {
    if (row.count === 0) {
      ...
      classes.forEach(c => {
        db.run('INSERT INTO classes (name, schedule, description) VALUES (?, ?, ?)', c);
      });
    }
  });
  ...
  db.get('SELECT COUNT(*) as count FROM trainers', (err, row) => {
    if (row.count === 0) {
      ...
      trainers.forEach(t => {
        db.run('INSERT INTO trainers (name, specialty, bio) VALUES (?, ?, ?)', t);
      });
    }
  });
});
...
app.post('/api/workouts', requireLogin, (req, res) => {
  ...
  db.run(
    ...
    [req.session.userId, exercise, sets, reps, weight, date],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ success: true });
    }
